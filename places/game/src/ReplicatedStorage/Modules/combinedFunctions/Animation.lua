...
--[[function animClass:Stop(fade: number?)
	...
	function animClass:Stop(fade: number?)
		for i,v in animator:GetPlayingAnimationTracks() do
			-- Compare the underlying AnimationId to reliably match the track
			local trackAnimId = v.Animation and v.Animation.AnimationId
			if trackAnimId == anim.AnimationId or v.Name == animClass.id then
				v:Stop(fade or 0)
			end
		end
	end
